...
SessionLayer runs in two modes: single-instance (the default: one process,
Postgres the only external dependency) and HA (multiple Control Planes and
Gateways, opt-in). This guide explains what HA adds, how routing and
failover actually behave, how to drain a Gateway gracefully, and what
surviving an instance loss does and does not mean.
...
- [ ] An L4 load balancer for the Gateways speaking PROXY protocol v2, and
      an L7 balancer for the Control Planes.
...
- Postgres: all durable, authoritative state, including *presence* (which
  Gateway currently owns each agent node's control channel, at what address,
  with a monotonically increasing nonce). Both modes use the same rows;
  only signaling differs.
- The coordination bus (NATS): transient signaling only. Its entire
  vocabulary is "deliver this dial-back request to the Gateway that owns
  node X, and carry the ingress Gateway's relay address." The owner is the
  party that dials.
- Direct Gateway↔Gateway connections: the session bytes. Established per
...
dropped. Ownership, relay routing and the TLS name a peer verifies are keyed
by the Gateway's enrolled *name*, so identity survives a restart or a
reschedule onto a new address.
...
- hands the killed instance's node ownership to standbys within the
  staleness window, and keeps new sessions establishing throughout.

The recovery story for the terminated sessions is deliberate: reconnecting
is cheap (a [pinned key](authentication.md) reconnects silently), lands on a
healthy instance, and re-runs full authorization. Plan for "reconnect in
seconds", not "sessions survive anything".

## Draining a Gateway

On `SIGTERM` the Gateway drains in order: its readiness endpoint flips to
503 while it *keeps accepting* for a short grace period (default 5 s) so
your load balancer deregisters it before it stops listening; then accept
loops stop, presence is released (standbys claim immediately), and agent
control channels close so Agents fail over. Live sessions, its own and the
relays it serves, get a bounded deadline (default 30 s) to finish; anything
still live is torn down through the recorder-finalize path, so no recording
is orphaned.

Point the LB health check at the Gateway's `/readyz`, and size probe
interval × unhealthy threshold to fit inside the pre-drain grace. Details
and the config keys are in the [Gateway runbook](../operations/gateway-runbook.md).

## The NATS bus in production

The bundled NATS client is minimal and deliberately dependency-free. It
connects in plaintext with no authentication, targeting a trusted internal
network. It reads the broker's `INFO` greeting, and if that advertises
`tls_required` or `auth_required` it logs one error and stops reconnecting
rather than downgrading.

That shapes where the TLS boundary can sit. The Gateway has to reach
something that speaks plaintext NATS, so the sidecar belongs beside each
Gateway, not in front of the broker:

- Run a TLS-terminating, authenticating NATS proxy or leaf node in the
  Gateway's own pod or on its host, listening on loopback.
- Point `ha.coordination.url` at that loopback address.
- The sidecar holds the client credential and speaks TLS to the broker, with
  subject authorization so only a node's owner may subscribe to its dial-back
  subject.

The remaining plaintext hop is loopback inside the Gateway's own trust
boundary, and Landlock plus the NetworkPolicy already confine what else can
reach it. A sidecar placed at the broker instead leaves the Gateway needing
to speak TLS itself, which is the case that fails closed.

> **Warning:** turning on TLS or authentication at the broker before the
> sidecars are in place takes HA signaling down fleet-wide. Every Gateway
> logs one loud error, stops reconnecting, and remote-owned sessions fail
> closed until the pairing is fixed. Move the Gateways onto sidecars first,
> then tighten the broker. Defense-in-depth behind the bus: owners drop
> stale-nonce and replayed signals, relay tokens are single-use and bound to
