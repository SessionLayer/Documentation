...
People and machines prove who they are two ways: to the Gateway when they
SSH, and to the Control Plane's REST API when they administer. This guide
covers the SSH method ladder, how pinning makes reconnects silent, how to
issue OTPs, and how to set up machine and first-admin access.
...
3. Pre-issued OTP: a one-time passcode, entered over keyboard-interactive.
...
never hard-fails the connection. The order is deliberate: certificate and
key methods are phishing-resistant and non-interactive, so they are primary;
the interactive device flow exists for first logins and browserless
recovery, not for every day.
...
A user with no credential runs `ssh`. The Gateway presents a
...
short TTL (60-300 seconds; default 120), at least 128 bits of entropy,
stored hashed, rate-limited, and the authenticated identity always comes
from the issuance record, never from anything the client types. The user
enters it at the keyboard-interactive prompt with echo off.

## Machine identities

Machines authenticate with OAuth client-credentials: no browser, no device
flow. Create a service account, issue it a credential, then exchange it for
...
A fresh install has empty RBAC, and default-deny would lock everyone out.
The one-time bootstrap resolves this in one of two ways:
