...
SessionLayer runs two separate authorization systems. [Data-plane
RBAC](../getting-started/concepts.md) decides who may SSH where, as which
Linux login, with which capabilities; platform RBAC decides who may
administer SessionLayer itself. They share no rules, no roles, and no code.
An admin who can edit rules does not thereby get SSH anywhere, and vice
versa.
...
> `sessionlayer.authz.max-grant-ttl` (default `PT1H`) and the identity's
> resolved max duration, and the smallest wins. The 28800 above therefore
> yields a one-hour grant on a stock install, not eight hours. The complete
> formula is in
> [Session limits](session-limits.md#what-bounds-a-sessions-duration).
...
identity's IdP groups appears in `groups`, or if `all` is `true`. An absent
or empty identity selector selects no one. A grant must name a subject
...
It narrows the one `allow` rule that carries it, and nothing else. Two
consequences to plan around:

- On a `deny` rule the condition is not evaluated at all. A deny applies on
  identity and node labels whatever the source. So "deny only from outside the
  office" cannot be written as a deny rule, and cannot be made to work as one:
  gating denies on source once meant an unknown or out-of-range source could
  drop a deny entirely, which turned source IP into positive evidence that
  removed a block. A deny that ignores source is what closes that hole. Such a
  rule denies everywhere, by design.
- A [JIT](jit-access.md) grant does not inherit it. An approved grant is
  evaluated as its own allow carrying no source condition, so a narrowing
  placed on a standing allow to the same node does not bind the JIT path.

A location control that must bind every path therefore belongs at the
Gateway's `ssh.source_ip_allowlist`, evaluated at TCP accept before any SSH
banner and so before rules, JIT, and break-glass alike
([Gateway configuration](../reference/config-gateway.md)). Use `permit_cidrs`
to narrow one standing grant; use the Gateway gate to bound the fleet.

## How a decision is made

At connect time the Control Plane evaluates the *set* of rules. The decision
is a pure function of that set, so rule order never matters and every
...
for admins and auditors. The allow decision is signed and handed to the
Gateway, which re-checks capability, expiry, and the lock set locally on
every channel open, so a `ControlMaster` connection multiplexing channels
for hours cannot outlive a policy change.
...
A rule that omits `capabilities` grants `shell` and `exec` only. For `shell`,
...
fails plainer, a channel-open or request failure with no reason detail, so
check the rule's capability list before debugging the network
...
- `port_forward_local` (`ssh -L`): the node dials the target, so a forward
  reaches only what the node itself can reach, the same network boundary
  your node-label rules already assume. A granted `-L` can still fail if the
  node cannot reach the target; the node, not the Gateway, makes the
  connection.
- `port_forward_remote` (`ssh -R`): the listener binds on the node, matching
  `-R` through a plain bastion. `-R 0:…` lets the node pick a port, which is
  reported back to your client; cancelling the forward unbinds the listener.
...
> unavailable there regardless of any grant. This is an architectural
> property of that mode, not a policy refusal. Remote forwarding and X11 are
> unaffected.

Agent forwarding is a different case entirely:

> **Warning:** the Gateway refuses SSH agent forwarding on every path,
> including ProxyJump, regardless of any `agent_forward` grant. This refusal
> is deliberate and permanent, and the inner-leg certificate never carries
> `permit-agent-forwarding` either. Forwarding your agent to a Tier-0
> intercepting proxy would hand it signing access to your private keys; the
> platform declines to be trusted that far.

## Platform RBAC

Platform RBAC is a separate system of roles (sets of granular permissions)
and role bindings (role → user or IdP group, optionally scoped). It is
default-deny, and every admin action is audited.
...
which a retired Gateway identity cannot be deleted.

Scoping (by node label, user, or time window) applies to the permissions
that read sensitive history, `recording:replay`, `recording:export`, and
