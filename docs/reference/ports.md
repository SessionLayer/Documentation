...
| NATS (optional, HA signals) | `4222` (client), `8222` (monitoring, dev) | NATS | Gateways, or their TLS sidecars | `ha.coordination.url` |
| NATS TLS sidecar (production HA) | `127.0.0.1:4222` by convention | NATS (plaintext, loopback) | the co-located Gateway only | `ha.coordination.url`; the sidecar's own config |
...
| Gateway (owner) | Gateway (ingress) | agent transport port | Direct peer byte relay for sessions landing on a non-owner Gateway (HA); bytes never cross the signal bus | `ha.peer_relay_advertise_addr` (empty derives from the agent transport) |
...
| Gateway | its own NATS TLS sidecar | `127.0.0.1:4222` | The same signaling, when the broker requires TLS or auth: the bundled client speaks neither, so the sidecar is co-located and the plaintext hop stays on loopback | `ha.coordination.url` |
| NATS TLS sidecar | NATS | `4222` | The authenticated, encrypted leg to the broker | the sidecar's own config |
