...
CLI definitions. This page restates them by hand; nothing compares the two.
...
> **Note:** `sessionlayer-agent run` refuses to start as root, and the check runs before any
> credential work: a root Agent could read the node's host key and impersonate the node. Run it
...
| `--join-token` | string (secret) | none | Token/OIDC join: the credential inline. Prefer the file form. |
| `--join-token-file` | path | none | Token/OIDC join: a file holding the credential. |
| `--operator-cert-file` | path | none | mTLS join: the operator certificate (PEM). Required with `--join-method mtls`. |
| `--operator-key-file` | path | none | mTLS join: the operator ECDSA P-256 key (PKCS#8 PEM). |
...
| `--self-blob-bundle` | path | none | The cosign blob-signature bundle for this binary. |
| `--self-provenance` | path | none | The SLSA provenance bundle for this binary. |
| `--self-trusted-root` | path | none | The pinned Sigstore `trusted_root.json`. |
