...
`gateway-core/src/config.rs`. This page restates it by hand; nothing compares the two.
...
- Empty means off. Listeners (`ssh.listen_addr`, `ssh.agent.listen_addr`, `ha.drain.readyz_addr`)
  are disabled when their address is empty.
...
| Key | Type | Default | Effect |
|---|---|---|---|
| `bootstrap.enrollment_token` | string (secret) | none | The single-use, short-TTL enrollment token (issued by the Control Plane). |
| `bootstrap.ca_cert_path` | path | none | PEM trust anchor the Gateway pins to verify the Control Plane's server certificate pre-enrollment. A wrong-CA server is refused. |
| `bootstrap.gateway_name` | string | none | The stable Gateway name the token was provisioned for; bound into the CSR and issued certificate. A mismatch fails closed. |
...
| `ssh.recorder.finalize_max_attempts` | int | `5` | Max attempts (including the first) for the FinalizeRecording call that commits the hash-chain head/digest after the object is already uploaded; retries with backoff on a transient Control Plane failure. There is no CP-side reaper for a lost commit, so this is retried harder than the upload. |
...
New privileged channels are refused once the grant expires in every mode. The three expiry modes:
...
| Key | Type | Default | Effect |
|---|---|---|---|
| `ssh.break_glass.enabled` | boolean | `true` | Whether this Gateway offers the break-glass authentication paths. Disabled, a break-glass credential does not resolve. |
...
| Key | Type | Default | Effect |
|---|---|---|---|
| `ssh.agent.listen_addr` | string | empty (disabled) | Listen address for the Agent transport. Off, an outbound-agent node is offline. Dev port is `9444`. |
...
| `ha.coordination.url` | string (URL) | none | NATS server URL (for example `nats://nats.internal:4222`). Required with the `nats` backend. |
...
With no subcommand the `gateway` binary runs the daemon. Two print-and-exit subcommands are useful
when diagnosing a node, and neither reads or needs a valid config file:

| Command | Effect |
|---|---|
| `gateway --config <PATH>` | Run the daemon against that config. Global: it also applies to the subcommands below. |
| `gateway health` | Print a JSON self-report and exit. |
...
`io-backend` answers the question the `io_backend` config key raises: a `uring` request degrades to
epoll wherever io_uring is unavailable, and this tells you which you would get *before* starting the
daemon, rather than inferring it from logs:

```console
$ gateway io-backend --request uring
requested Uring -> resolved Epoll
```
