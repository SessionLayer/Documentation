...
| `outcome` | The per-action result. A closed set of four: `success`, `denied`, `failure`, `error`. A denial is `denied`, never `deny`. |
...
A denied `gateway.renew` or `agent.renew` carries its reason in the event detail rather than in the
action name. `fingerprint_mismatch` is the one to alert on: the presented certificate is neither the
current nor the previous generation, which is what a restored-from-backup Control Plane looks like
to a running component ([Disaster recovery](../operations/disaster-recovery.md)).
