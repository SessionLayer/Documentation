...
restated here by hand, with nothing comparing the two. Set them the standard Spring Boot ways: `application.properties`, a config
...
| `sessionlayer.mtls.enrollment-token-ttl` | duration | `PT10M` | TTL of the single-use Gateway enrollment token, when `POST /v1/gateway-enrollment-tokens` omits `ttlSeconds`. |
| `sessionlayer.mtls.enrollment-token-max-ttl` | duration | `PT1H` | Ceiling on an operator-supplied `ttlSeconds`. A longer request is clamped to this, not rejected. |
...
| `sessionlayer.agent-join.renewal-receipt-ttl` | duration | `PT5M` | How long a completed `RenewAgentIdentity` result stays replayable to the *same* CSR key. If an Agent's renewal succeeds but its response is lost, the retry presents a now-stale generation; without this window the Control Plane cannot tell that from a cloned credential and locks the identity with no auto-clear. Only needs to outlive an RPC deadline plus one backoff. Keep it short: a longer window widens how long a stolen credential could replay a renewal. A retry with a *different* CSR key is still treated as a clone, whatever this is set to. |
...
| `sessionlayer.agent-join.oidc.issuer` | string | none | The workload issuer URL the token's `iss` must equal. |
| `sessionlayer.agent-join.oidc.jwks-uri` | string | none | The issuer's JWKS URI (public signing keys). |
| `sessionlayer.agent-join.oidc.audience` | string | none | The audience the token's `aud` must contain. |
...
| `sessionlayer.agent-join.mtls.operator-ca-pem` | string (PEM) | none | The operator CA trust anchor(s) an operator certificate must chain to. |
...
| `sessionlayer.bootstrap.admin-subject` | string | none | An OIDC subject to provision as the initial platform admin. Unset ⇒ a printed-once bootstrap credential is generated instead. |
...
| `sessionlayer.oidc.issuer` | string | none | The IdP issuer URL the ID token's `iss` must carry. |
| `sessionlayer.oidc.client-id` | string | none | This Control Plane's OIDC client id (the required `aud`). |
| `sessionlayer.oidc.client-secret` | string | none | The client secret, if the IdP requires one at the token endpoint. |
| `sessionlayer.oidc.redirect-uri` | string | none | The public auth-code callback URL. Its origin is also the single source of the device-flow verification URL, never a request `Host` header. |
...
| `sessionlayer.rest-security.basic-auth.username` | string | none | The single operator username. |
| `sessionlayer.rest-security.basic-auth.password-hash` | string | none | BCrypt hash of the operator password, never a raw password. |

## REST request deadline (`sessionlayer.web.*`)

| Key | Type | Default | Effect |
|---|---|---|---|
| `sessionlayer.web.request-timeout` | duration | `PT15S` | Deadline on the whole REST/WebFlux request chain, matching `sessionlayer.mtls.rpc-timeout`'s own default: a query wedged past connection acquisition (a lock wait, a stalled scan) returns a clean `504` instead of hanging the caller forever. |
...
## Azure Key Vault CA backend (`sessionlayer.ca.azure.*`)

Adopts a `user`, `session`, or `host` CA onto Azure Key Vault; unset, every CA stays on `local`. See
[Certificate authorities](../admin-guides/certificate-authorities.md#adopt-key-vault-for-a-ca) for
the adoption procedure and the minimum Azure role.
