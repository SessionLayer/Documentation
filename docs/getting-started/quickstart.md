...
Control Plane and Gateway installed from their signed release artifacts,
Postgres, a write-once (WORM) object store (MinIO), one node running plain
OpenSSH, and the platform's real REST API for every step.
...
> a verification failure fails the build (see [Supply
> chain](../security/supply-chain.md), and `cp.Dockerfile`/
> `gateway.Dockerfile` in this directory for the verify step itself). The
> first run takes a few minutes of unattended time to pull and verify the
> images, far faster than a from-source build, and the 20 minutes of your
> attention starts after it. Subsequent starts reuse the images and take
> seconds. To track a newer release, pass `--build-arg CP_TAG=`/`GW_TAG=`
> (see `compose.yaml`).
...
workstation), and a one-shot `seed`. The seed writes five things straight to
the database so this walkthrough can start at the interesting part: the CA
trust anchors (the Gateway's mTLS anchor, the session CA line the node
trusts, the host CA public key), a single-use Gateway enrollment token, the
`quickstart-admin` service account and its credential, and a demo
[customer recording key](concepts.md). The Control Plane gets only that key's
public half; the private half stays on a dedicated local volume that only the
decrypt tool mounts.

> **Note:** a real install does all of that over the API, and needs no
> database credential for any of it: the trust anchors and CA keys come from
> `/v1/cas`, the enrollment token from `/v1/gateway-enrollment-tokens`, and
> the recording key from `/v1/operator-settings/recording-customer-key`. The
> shortcut here is that the seed skips the one genuinely first step, which is
> claiming the bootstrap credential and minting a machine credential from it.
> [Install the Control Plane](../installation/control-plane.md) walks that
> path in order.
...
credential to its log. Whoever surrenders it becomes the first platform
admin, and the bootstrap self-disables. Fish it out and claim it:
...
You should see `{"status":"provisioned"}`. `quickstart-admin` is now
platform admin, the claim is audited, and this endpoint is dead from here on.
...
minted per connection, only after the rule above says yes.
...
Deny is equally real. `dba` is a valid login on the node, but no rule grants
it, so:
...
TOKEN=$(curl -s http://127.0.0.1:8080/v1/oauth2/token -H 'Content-Type: application/json' \
  -d '{"grant_type":"client_credentials","client_id":"quickstart-admin","client_secret":"quickstart-admin-dev-secret"}' | jq -r .access_token)
until curl -s "http://127.0.0.1:8080/v1/recordings?sessionId=$SESSION" \
  -H "Authorization: Bearer $TOKEN" \
  | jq -e '.items[0].status == "finalized"' >/dev/null; do sleep 2; done
REC=$(curl -s "http://127.0.0.1:8080/v1/recordings?sessionId=$SESSION" \
...
`SLREC1` is a sealed object, not a terminal transcript. This is the
crown-jewel property: the recording is encrypted to the customer recording
key, and the platform stores only the public half. Nobody with Control Plane
access, object-store access, or a platform admin role can read it. Decrypt it
yourself with the private half (the seed left the demo key on a dedicated
volume that only the decrypt tool mounts; the decryptor is a small offline
tool shipped with this example). Build it first: `docker compose run` prints
build progress on stdout, which would land in `session.cast` and corrupt it.
...
```text
{"height":24,"timestamp":1785177491,"version":2,"width":80}
[0.293425996,"i","echo hello from $(hostname); id -un"]
```

That is a standard [asciicast v2](https://docs.asciinema.org/manual/asciicast/v2/)
file: a header line, then timestamped output and keystroke events. Play it
with any asciinema-compatible player. For a quick look, render only the
terminal output:
...
- [Core concepts](concepts.md): the architecture you exercised, in ten
  minutes.
- [SSH access](../user-guide/ssh-access.md): addressing modes,
  `~/.ssh/config`, ProxyJump with `@cert-authority`, all against this stack.
- [Requesting access](../user-guide/requesting-access.md): walk a JIT
  request from denied to approved on this stack.
- [Production hardening](../security/hardening.md): everything this
  evaluation deliberately relaxed.
