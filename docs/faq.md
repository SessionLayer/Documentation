...
Short, direct answers. Where a question deserves depth, the answer links to
the page that has it.

## Can SessionLayer staff or a platform admin read my recordings?

No. Recordings are encrypted before upload to a public key whose private half
only you, the operator, hold. The platform stores the public half and the
...
Yes. The Gateway is a fully-trusted intercepting proxy: zero trust describes
the access-decision model, not the absence of a trusted component, and that
is what makes recording and file-transfer audit possible. See the
[Trust model](security/trust-model.md) for the full argument, including the
plaintext-visible trade-off.

## Does it support Kubernetes, databases, RDP, or web apps?

No. SessionLayer is SSH-only, for a fleet of Linux nodes, from stock OpenSSH
clients. There is no kubectl proxying, database access, or application
gateway, and none is documented here because none exists. See
[How SessionLayer compares](getting-started/how-it-compares.md).

## What happens when the Control Plane is down?

Existing sessions keep running: their bytes flow Gateway to node directly.
New sessions fail closed with "service temporarily unavailable". Locks keep
working, since they are pushed to Gateways ahead of time precisely so
revocation survives a datastore loss, and a Gateway that cannot verify its
...
standing path to a node. Nodes trust only the platform's session CA, and
every session gets a fresh short-lived certificate minted after policy
...
and `ControlMaster` all work. Setup is a few lines of `~/.ssh/config` (or a
...
cannot be moved), and no such feature is planned around the edges of
physics. The design instead makes reconnection cheap (pinned-key silent
reconnect) and failure bounded: planned restarts drain gracefully, and in HA
the rest of the fleet keeps serving while ownership fails over. See
...
compliance mode nobody, including the platform, can delete before retention
expires. Replay, export, and delete are themselves audited. The limit:
without the deferred external Merkle anchor, a full database
superuser who also defeats the append-only trigger could rewrite the audit
chain at the source. That is why the off-box SIEM forward and the restricted
DB role are go-live preconditions. See the
...
cannot read your recordings either. Store the private key like the crown
jewel it is. See [Production hardening](security/hardening.md).
...
bypasses approval chains and the identity provider, never denial. Every use
alarms, forces strict recording, and requires review. See
...
verifying release signatures, a pinned Sigstore trust root checked offline.
...
- [Core concepts](getting-started/concepts.md): the vocabulary behind these
  answers.
- [Trust model](security/trust-model.md): the full threat model, stated
  plainly.
